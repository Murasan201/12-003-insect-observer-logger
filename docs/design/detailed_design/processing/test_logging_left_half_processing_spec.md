# test_logging_left_half.py 処理説明書

**文書番号**: 12-002-PROC-011
**プロジェクト名**: 昆虫自動観察＆ログ記録アプリ
**文書名**: test_logging_left_half.py 処理説明書
**対象ファイル**: `production/test_logging_left_half.py`
**バージョン**: 1.0
**作成日**: 2025-12-25
**作成者**: 開発チーム

---

## 📋 ファイル概要

### 目的
Raspberry Pi Camera Module 3 Wideを使用した長時間昆虫検出ロギングスクリプト。画面左半分のみを検出エリアとして使用し、誤検出を低減しながら長時間の自動観測を実現する。

### 主要機能
- 画面左半分（50%）のみを検出エリアとして使用
- 指定間隔での自動撮影・検出
- CSV形式での検出ログ記録
- JSON形式でのセッションメタデータ保存
- 検出時の画像自動保存（オプション）
- オートフォーカス/マニュアルフォーカス対応
- カメラパラメータの詳細設定

### 本番環境での使用実績

**重要**: 本ファイルは本番環境で実際に使用されたオリジナルスクリプトです。**編集禁止**です。

---

## 🔧 本番環境で使用したコマンド

### 9時間の本番ロギング（実証済み最適パラメータ）

以下のコマンドは実際の本番環境で使用され、長時間データ収集に成功したものです。

```bash
cd production/

nohup python3 test_logging_left_half.py \
  --auto-focus \
  --conf 0.4 \
  --exposure -0.5 \
  --contrast 2 \
  --brightness -0.05 \
  --interval 60 \
  --duration 32400 \
  --save-images > logging_9h.log 2>&1 &
```

#### パラメータ説明

| パラメータ | 値 | 説明 |
|-----------|-----|------|
| `--auto-focus` | - | オートフォーカスを有効化 |
| `--conf` | 0.4 | 検出信頼度閾値（40%以上で検出と判定） |
| `--exposure` | -0.5 | 露出補正（やや暗めに設定） |
| `--contrast` | 2 | コントラスト強調 |
| `--brightness` | -0.05 | 明るさ微調整 |
| `--interval` | 60 | 観測間隔（60秒） |
| `--duration` | 32400 | 総観測時間（32400秒 = 9時間） |
| `--save-images` | - | 検出時に画像を保存 |

### 収集されたデータ

| ファイル | 期間 | レコード数 |
|----------|------|-----------|
| `left_half_detection_log_20250902_220517.csv` | 約8時間 | 478件 |
| `left_half_detection_log_20250903_204658.csv` | 約9時間 | 538件 |
| `left_half_detection_log_20250904_210012.csv` | 約9時間 | 538件 |

---

## 💻 コマンドライン引数

### 基本構文
```bash
python3 test_logging_left_half.py [options]
```

### 引数一覧

| 引数 | 型 | デフォルト | 説明 |
|------|---|-----------|------|
| `--interval` | int | 60 | 観測間隔（秒） |
| `--duration` | int | 3600 | 総観測時間（秒） |
| `--conf` | float | 0.3 | 検出信頼度閾値 |
| `--save-images` | flag | False | 検出時に画像を保存 |
| `--auto-focus` | flag | False | オートフォーカス有効化 |
| `--focus-distance` | float | 30 | マニュアルフォーカス距離（cm） |
| `--exposure` | float | 0 | 露出補正値 |
| `--contrast` | float | 1.0 | コントラスト値 |
| `--brightness` | float | 0 | 明るさ補正値 |
| `--output-dir` | str | insect_detection_logs | 出力ディレクトリ |

---

## 📤 出力仕様

### CSVファイル

**ファイル名パターン**: `left_half_detection_log_{YYYYMMDD_HHMMSS}.csv`

**カラム定義**:

| カラム名 | 型 | 説明 |
|----------|---|------|
| timestamp | str | ISO 8601形式タイムスタンプ |
| observation_number | int | 観測番号 |
| detection_count | int | 検出数 |
| has_detection | bool | 検出有無 |
| class_names | str | 検出クラス名 |
| confidence_values | str | 信頼度値 |
| bbox_coordinates | str | バウンディングボックス座標 |
| center_x | float | 検出中心X座標 |
| center_y | float | 検出中心Y座標 |
| bbox_width | float | バウンディングボックス幅 |
| bbox_height | float | バウンディングボックス高さ |
| area | float | 検出面積 |
| detection_area | str | 検出エリア（left_half） |
| processing_time_ms | float | 処理時間（ミリ秒） |
| image_saved | bool | 画像保存有無 |
| image_filename | str | 保存画像ファイル名 |

### メタデータJSON

**ファイル名パターン**: `left_half_metadata_{YYYYMMDD_HHMMSS}.json`

**Git管理対象ファイル**: `left_half_metadata_20250904_210012.json`（本番パラメータ記録）

**フォーマット定義**:

```json
{
  "start_time": "2025-09-04T21:00:12.074679",
  "detection_area": "left_half",
  "area_description": "Only left 50% of camera view is monitored",
  "log_file": "path/to/log_file.csv",
  "system_info": {
    "camera": "Camera Module 3 Wide NoIR",
    "platform": "Raspberry Pi"
  },
  "focus_mode": "auto",
  "focus_distance_cm": null,
  "model_path": "../weights/best.pt",
  "confidence_threshold": 0.4,
  "resolution": "2304x1296",
  "detection_width": 1152,
  "interval_seconds": 60,
  "duration_seconds": 32400,
  "save_images": true
}
```

**フィールド定義**:

| フィールド | 型 | 説明 |
|-----------|---|------|
| start_time | str | セッション開始時刻（ISO 8601） |
| detection_area | str | 検出エリア（left_half） |
| area_description | str | エリアの説明 |
| log_file | str | 出力CSVファイルパス |
| system_info | object | システム情報（camera, platform） |
| focus_mode | str | フォーカスモード（auto/manual） |
| focus_distance_cm | float/null | マニュアルフォーカス距離 |
| model_path | str | YOLOモデルパス |
| confidence_threshold | float | 検出信頼度閾値 |
| resolution | str | カメラ解像度 |
| detection_width | int | 検出エリア幅（ピクセル） |
| interval_seconds | int | 観測間隔（秒） |
| duration_seconds | int | 総観測時間（秒） |
| save_images | bool | 画像保存フラグ |

---

## 📊 本番環境の実行例

### テスト実行（1時間）

```bash
python3 test_logging_left_half.py \
  --auto-focus \
  --conf 0.3 \
  --interval 30 \
  --duration 3600 \
  --save-images
```

### 短時間テスト（10分）

```bash
python3 test_logging_left_half.py \
  --auto-focus \
  --interval 10 \
  --duration 600 \
  --save-images
```

### プロセス管理

```bash
# 実行状況確認
ps aux | grep test_logging

# ログのリアルタイム監視
tail -f logging_9h.log

# CSVデータ確認
tail -10 production/insect_detection_logs/*.csv
```

---

## 🔗 依存関係

### 必須ライブラリ
- picamera2
- libcamera
- opencv-python (cv2)
- numpy
- ultralytics (YOLO)

### ハードウェア要件
- Raspberry Pi 4/5
- Camera Module 3 Wide（推奨）
- 十分なストレージ容量（長時間ロギング時）

---

## ⚠️ 注意事項

### ファイル保護ポリシー

**本ファイル（`test_logging_left_half.py`）は本番環境で使用されたオリジナルスクリプトです。**

- **絶対に編集しないでください**
- 変更が必要な場合は `production_logging_left_half.py`（書籍掲載用コピー）を編集してください
- ログデータ（CSV/JSON）も同様に編集禁止です

---

## 📝 更新履歴

| バージョン | 日付 | 更新内容 |
|-----------|------|---------|
| 1.0 | 2025-12-25 | 初版作成 |

---

*文書番号: 12-002-PROC-011*
*最終更新日: 2025-12-25*
